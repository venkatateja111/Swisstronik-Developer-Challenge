...
async function main() {
    ...
    const functionName = "isRegisteredVoter";
    const voterAddress = hre.ethers.getAddress(addr1.address);
    const responseMessage = await sendShieldedQuery(signer.provider, contractAddress, contract.interface.encodeFunctionData(functionName, [voterAddress]));
